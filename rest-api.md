...
feein | boolean | NO | fee inclusive, deduct the fee from the transfer amount<br />0: (default) don't deduct the fee from the transfer amount<br />1: deduct the fee from the transfer amount
uid | string | YES |User ID
ts | number | YES |Unix time
sig | string | YES |HMAC-SHA256 signature
