...
**IMPORTANT 1:** If there was an error during IPN callback, our system will try upto 5 times until it is done successfully and nevertheless there is a tiny chance your system receives more than one IPN callback for the same transaction. Your IPN callback handler must alway watch for duplicate transaction by checking ```zxid``` (ZixiPay transaction id) to avoid double deposit/credit on your side.
...
**IMPORTANT 3:** Callback is HTTPS only, so SSL needs to be enabled and valid on the receiving end.
